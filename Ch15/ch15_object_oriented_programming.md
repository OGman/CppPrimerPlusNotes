...
- 虚析构函数将阻止合成移动操作
### 15.7.2 合成拷贝控制与继承
合成的拷贝控制成员对类本身的成员依次进行初始化、赋值、销毁的操作，然后使用直接基类中的对应操作对其直接基类部分进行初始化、赋值、销毁操作

### 15.7.3 派生类的拷贝控制成员
构造函数、拷贝构造函数、移动构造函数、赋值运算符既要控制对象本身的成员，也负责控制基类部分的成员。析构函数只负责销毁类本身的成员
- 定义派生类的拷贝或移动构造函数
- 派生类赋值运算符
- 派生类析构函数
- 在构造函数和析构函数中调用虚函数
派生类对象的基类部分将首先被构建，此时派生类部分是未初始化状态。析构时，派生类部分首先被销毁，然后析构基类部分。两种函数执行过程中，对象都有一段时间处于未完成状态，这一状态的正确处理是重要的。
重要的原则是，我们构建一个对象时，需要把对象的类和构造函数的类看成同一个
### 15.7.4 继承的构造函数
类不能继承默认、拷贝和移动构造函数
...
## 15.8 容器与继承
当我们使用容器保存继承体系中的对象时，通常必须采用间接存储的方式（保存指针）
- 在容器中放置（智能）指针而非对象
通常在容器中放置基类的指针
### 15.8.1 编写Basket类
``` C++
